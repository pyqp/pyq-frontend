...
import {
  ChevronLeft, ChevronRight, Flag, Clock, Send,
  BookmarkCheck, AlertTriangle, CheckCircle, Circle, Loader2
} from 'lucide-react';
...
const Timer = ({ seconds, warn }: { seconds: number; warn: boolean }) => (
  <div className={`flex items-center gap-2 px-4 py-2 border-4 border-black font-black text-2xl tabular-nums ${warn ? 'bg-red-500 text-white animate-pulse' : 'bg-white text-black'}`}>
...
export default function TestPage() {
  ...
  useEffect(() => {
    if (!attempt) return;
    setStates(attempt.questions.map(() => ({ answer: null, marked: false, timeSpent: 0 })));
  ...
  const saveToBackend = useCallback((qIdx: number, answer: number | null, marked: boolean) => {
  ...
  const handleSubmit = useCallback(async (auto = false) => {
    ...
    try {
      const { data } = await mockTestApi.submitTest(id, { attemptId: attempt.attemptId, timeTaken });
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <p className="font-black text-xl" style={{ fontFamily: "'Archivo Black',sans-serif" }}>Loading test...</p>
        </div>
      </div>
    );
  }
  ...
  const notVisited  = states.filter((x, i) => x.answer === null && !x.marked && i !== current).length;

  return (
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col" style={{ fontFamily: "'Space Grotesk',sans-serif" }}>
      ...
      <header className="bg-black text-white px-4 py-3 flex items-center justify-between border-b-4 border-black sticky top-0 z-40">
        <div>
          <p className="text-gray-400 text-xs font-medium uppercase tracking-widest">Mock Test</p>
          <p className="font-black text-lg leading-tight" style={{ fontFamily: "'Archivo Black',sans-serif" }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="px-3 py-1 bg-blue-100 text-blue-800 border-2 border-blue-300 font-bold text-xs uppercase">{q.subject}</span>
            {q.topic && <span className="px-3 py-1 bg-gray-100 text-gray-600 border-2 border-gray-300 font-bold text-xs uppercase">{q.topic}</span>}
            <span className={`px-3 py-1 border-2 border-black font-bold text-xs uppercase ${q.difficulty === 'easy' ? 'bg-green-300' : q.difficulty === 'hard' ? 'bg-red-300' : 'bg-yellow-300'}`}>{q.difficulty}</span>
            <span className="ml-auto text-sm font-bold text-gray-500">+{q.marks} / -{q.negativeMarks ?? 0}</span>
          </div>

          {/* Question text */}
          <div className="bg-white border-4 border-black p-6 mb-6 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            {q.imageUrl && <img src={q.imageUrl} alt="question" className="max-w-full mb-4 border-2 border-gray-200" />}
            <p className="text-lg font-bold leading-relaxed text-black">{q.questionText}</p>
          </div>
          ...
          <div className="space-y-3 mb-8">
            {q.options.map((opt, idx) => {
              ...
              return (
                ...
                >
                  <span className={`w-8 h-8 flex-shrink-0 flex items-center justify-center border-2 font-black text-sm ${selected ? 'border-white bg-white text-black' : 'border-black'}`}>
                    {String.fromCharCode(65 + idx)}
                  </span>
                  <span className="pt-0.5">{opt.text}</span>
                  {opt.image && <img src={opt.image} alt={`option ${idx}`} className="max-h-16 ml-auto" />}
                </button>
              );
            })}
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button onClick={handleMark}
              ...
              <Flag className="w-4 h-4" />{s.marked ? 'Unmark' : 'Mark for Review'}
            </button>
        ...
        <aside className="w-72 border-l-4 border-black bg-white overflow-y-auto flex-shrink-0 hidden md:flex flex-col">
          ...
          <div className="p-4 border-b-4 border-black">
            <p className="font-black text-xs uppercase tracking-widest mb-3">Question Status</p>
            <div className="grid grid-cols-2 gap-2 text-xs font-bold">
              <div className="flex items-center gap-1"><span className="w-5 h-5 bg-green-500 border-2 border-green-600 inline-block" /><span>{answered} Answered</span></div>
              <div className="flex items-center gap-1"><span className="w-5 h-5 bg-yellow-400 border-2 border-yellow-500 inline-block" /><span>{markedCount} Marked</span></div>
              <div className="flex items-center gap-1"><span className="w-5 h-5 bg-white border-2 border-gray-300 inline-block" /><span>{notVisited} Not visited</span></div>
              <div className="flex items-center gap-1"><span className="w-5 h-5 bg-purple-500 border-2 border-purple-600 inline-block" /><span>Ans+Mark</span></div>
            </div>
          </div>
          ...
          <div className="p-4 grid grid-cols-5 gap-2">
            {questions.map((_, idx) => {
              const st = states[idx] ?? { answer: null, marked: false, timeSpent: 0 };
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] p-8 max-w-md w-full">
            ...
            <h2 className="font-black text-2xl mb-2" style={{ fontFamily: "'Archivo Black',sans-serif" }}>Submit Test?</h2>
            <p className="text-gray-600 font-medium mb-6">This cannot be undone.</p>

            <div className="grid grid-cols-2 gap-4 mb-6 text-center">
              <div className="p-3 bg-green-50 border-2 border-green-400">
                <div className="flex items-center justify-center gap-1 text-green-700 font-black text-2xl"><CheckCircle className="w-5 h-5" />{answered}</div>
                <p className="text-xs font-bold text-gray-500 uppercase">Answered</p>
              </div>
              <div className="p-3 bg-gray-50 border-2 border-gray-300">
                <div className="flex items-center justify-center gap-1 text-gray-600 font-black text-2xl"><Circle className="w-5 h-5" />{questions.length - answered}</div>
                <p className="text-xs font-bold text-gray-500 uppercase">Unanswered</p>
              </div>
              {markedCount > 0 && (
                <div className="col-span-2 p-3 bg-yellow-50 border-2 border-yellow-400">
                  <div className="flex items-center justify-center gap-1 text-yellow-700 font-black text-lg"><BookmarkCheck className="w-4 h-4" />{markedCount} marked for review</div>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-4">
              ...
              <button onClick={() => handleSubmit(false)} disabled={submitting}
                ...
                {submitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Confirm Submit'}
              </button>
            </div>
          </div>
        </div>
